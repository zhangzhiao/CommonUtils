 ...
 */
object CommonUtil {
    val loopCache = hashMapOf<Int, Runnable>()
    val loopTaskCache = hashMapOf<Int, Task<*>>()
    ...
    private val mainThread: Handler by lazy {
        Handler(Looper.getMainLooper()) {
            loopCache[it.what]?.let { runnable ->
     ...
     * 轮询任务
     */
    fun <T> loop(delay: Long, loopKey: Int, task: Task<T>) {
        val loopRunnable = Runnable {
            mainThread.post(task.runnable)
            mainThread.sendEmptyMessageDelayed(loopKey, delay)
        }
        loopTaskCache[loopKey] = task
        loopCache[loopKey] = loopRunnable
        mainThread.post(loopRunnable)
    }
...
) = CommonUtil.Task(this, err, run = run)
...
fun removePoll(loopKey: Int) {
    CommonUtil.loopCache[loopKey]?.let {
        CommonUtil.removeRunnable(it)
        CommonUtil.loopCache.remove(loopKey)
    }
    CommonUtil.loopTaskCache[loopKey]?.let {
        CommonUtil.remove(it)
        CommonUtil.loopTaskCache.remove(loopKey)
    }
